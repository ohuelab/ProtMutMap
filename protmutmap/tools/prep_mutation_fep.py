...
class DefaultProteinMutationGenerator:
    ...
    charge_nrep: int = 64
    extreme_nrep: int = 128
    keep_nonmutated_rotamers: bool = False
    default_nrep: Optional[int] = None
    multi_mutation_multiplier: Optional[float] = None
    ...
    def __init__(self, pdb: str, gmx: str, python3: str, faspr: str, fepsuite: str,
        ...
        self.multi_mutation_multiplier = multi_mutation_multiplier
    ...
    def generate_mutant(self, basepdbinfo: PDBInfoSummary, ddir: str, mutations: Sequence[Mutation]):
        with open(f"{ddir}/seq.txt", "w") as ofh:
            ...
            for (c, r) in pdborder:
                ...
                # When requested, keep all non-mutated rotamers (e.g. for simulation-derived structures)
                elif self.keep_nonmutated_rotamers and not mutated:
                    res = res.lower()
    ...
    def generate_para_conf(self, muts: Sequence[Mutation],
                          ...
                          multi_mutation_multiplier: Optional[float] = None):
        """
        Generate para_conf.zsh file with NREP setting.

        Uses lambda_calculator to determine the appropriate number of replicas,
        taking into account mutation difficulty, charge changes, and number of mutations.

        Args:
            muts: Sequence of Mutation objects
            default_nrep: Number of replicas for easy mutations (default: self.default_nrep or DEFAULT_NREP)
            difficult_nrep: Number of replicas for difficult/bulky mutations (default: self.difficult_nrep)
            charge_nrep: Number of replicas for charge-changing mutations (default: self.charge_nrep)
            extreme_nrep: Number of replicas for extreme mutations (default: self.extreme_nrep)
        """
        para_conf_content = generate_para_conf_content(
            muts,
            default_nrep=default_nrep if default_nrep is not None else (self.default_nrep if self.default_nrep is not None else DEFAULT_NREP),
            difficult_nrep=difficult_nrep if difficult_nrep is not None else self.difficult_nrep,
            charge_nrep=charge_nrep if charge_nrep is not None else self.charge_nrep,
            extreme_nrep=extreme_nrep if extreme_nrep is not None else self.extreme_nrep,
            multi_mutation_multiplier=multi_mutation_multiplier if multi_mutation_multiplier is not None else (self.multi_mutation_multiplier if self.multi_mutation_multiplier is not None else MULTI_MUTATION_MULTIPLIER),
            always_write=True
        )

        # Write para_conf.zsh file
        with open("para_conf.zsh", "w") as ofh:
            ofh.write(para_conf_content)
    ...
    def generate(self, muts:Union[str, HashableMutations], muts_name: Optional[str], base_muts: Union[str, HashableMutations] = "", base_name: Optional[str] = None, base_mut_separator: str = "_") -> Tuple[str, List[str]]:
        ...
        for (_mode, dirname, mutstr_or_data) in \
            ...
            if isinstance(mutstr_or_data, str):
                if mutstr_or_data == "":
                    mutation_list = []
                else:
...
def argparse_options():
    ...
    parser.add_argument("--default-nrep", default=None, type=int, help=f"Num of replica for easy mutations (default: {DEFAULT_NREP} from lambda_calculator)")
    parser.add_argument("--difficult-nrep", default=64, type=int, help="Num of replica on difficult (involving YFWP) cases")
    parser.add_argument("--charge-nrep", default=64, type=int, help="Num of replica on charge-changing (non-bulky) cases")
    parser.add_argument("--extreme-nrep", default=128, type=int, help="Num of replica on extreme (both bulky/aromatic AND charge-changing) cases")
    parser.add_argument("--multi-mutation-multiplier", default=None, type=float, help=f"Multiplier for each additional mutation beyond the first (default: {MULTI_MUTATION_MULTIPLIER} from lambda_calculator)")
    parser.add_argument("--keep-nonmutated-rotamers", action="store_true", default=False,
                        help="Keep original rotamers for non-mutated residues (recommended for simulation-derived input structures)")
...
if __name__ == "__main__":
    ...
    generator = DefaultProteinMutationGenerator(args.pdb, args.gmx, args.python3, args.faspr, args.fepsuite,
                                                ...
                                                multi_mutation_multiplier=args.multi_mutation_multiplier,
