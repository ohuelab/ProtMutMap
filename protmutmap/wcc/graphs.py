...
class Graph:
    def __init__(self, filename=None, dataframe=None, mol1_col='mol1', mol2_col='mol2', ddg_col='ddG', uncertainty_col=None):
        self.num_e=0
        ...
        if dataframe is not None:
            self._load_from_dataframe(dataframe, mol1_col, mol2_col, ddg_col, uncertainty_col)
        elif filename is not None:
            self._load_from_file(filename)
        else:
            print("Error: No input files or dataframe provided")
            exit()
    ...
    def _load_from_dataframe(self, df, mol1_col, mol2_col, ddg_col, uncertainty_col=None):
        required_cols = [mol1_col, mol2_col, ddg_col]

        # Check if required columns exist
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            raise Exception(f"DataFrame must contain columns: {missing_cols}. Available columns: {list(df.columns)}")

        # Check if uncertainty_col is specified but doesn't exist
        extra_cols = [col for col in df.columns if col not in required_cols]
        uncertainty_col_missing = False
        if uncertainty_col is not None and uncertainty_col not in extra_cols:
            warnings.warn(f"Uncertainty column '{uncertainty_col}' not found in DataFrame. Available columns: {list(df.columns)}. Proceeding without weights (weight='no').", UserWarning)
            uncertainty_col_missing = True

        for _, row in df.iterrows():
            # Skip edges with missing ddG (e.g., incomplete FEP calculations)
            if pd.isna(row[ddg_col]):
                continue
            # Extract required columns by name
            data = [str(row[mol1_col]), str(row[mol2_col]), str(row[ddg_col])]

            # If uncertainty_col is specified and exists, place it first, then add other columns
            if uncertainty_col is not None and not uncertainty_col_missing and uncertainty_col in extra_cols:
                # Add uncertainty column first
                data.append(str(row[uncertainty_col]))
                # Add remaining columns
                for col in extra_cols:
                    if col != uncertainty_col:
                        data.append(str(row[col]))
            elif uncertainty_col_missing:
                # uncertainty_col was specified but doesn't exist - don't add any weights
                weight = "no"
            else:
                # uncertainty_col not specified - add columns in order they appear (original behavior)
                for col in extra_cols:
                    data.append(str(row[col]))

            if not uncertainty_col_missing:
                if len(data)==3:
                    weight="no"
                elif len(data)>3:
                    weight="yes"
